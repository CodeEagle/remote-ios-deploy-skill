...
- `python3 scripts/bridge-auto.py <overlay-ip>` is the resident form of the bridge — the only required input is the phone's overlay address (`PHONE` env or `argv[1]`); `LOCAL_IP` defaults to the IPv4 of the default-route interface. It probes the phone's control port, publishes the Bonjour proxy record, lays the control relay, and runs the same drift-following / prefetch / idle-reap loop as `bridge-dynamic.py` (`PREFETCH` 8, `IDLE_TTL` 600s, `CLEAN_INTERVAL` 60s), restarting `dns-sd` and the `log stream` watcher if either dies. One-time prerequisite it cannot create for you: the phone must already be Xcode-paired with **this** Mac (Trust tapped, passcode entered) — see [references/first-time-setup.md](references/first-time-setup.md).
  ...
  Also verified end to end after a live takeover: the launchd-managed resident bridge on this Mac (`com.iris.remote-ios-bridge`) was booted out, `bridge-auto.py` was started with the phone's real Bonjour identity, and Xcode on another LAN host reconnected through the new bridge within seconds — `remotepairingd` logged fresh tunnel endpoints, the `log stream` watcher raised relays on the drifting port plus its `PREFETCH` window, and installs flowed again. Full SIGTERM of the live bridge exits in ~3s leaving zero `dns-sd` / `log stream` / socat-listener orphans; relays torn down mid-session are re-laid automatically on the next endpoint event.

  Not tested this session: repeated long-run stability across phone Wi-Fi switches and `remotepairingd` restarts — the identity-rotation caveat above still applies, and this session's takeover used a fixed real identity. socat forks one child per accepted connection — child count tracks live connections (observed ~70 under active Xcode use), while the listener count is what the idle-reap keeps in the tens.
